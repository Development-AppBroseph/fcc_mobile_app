...
class _ChangePlanPageState extends State<ChangePlanPage> {
  ...
  String? checkMembershipPrice(String id) {
    ...
    if (user is Authenticated) {
      ...
      if (parsedUserDiscount < parsedMembershipPriceById) {
        final String amount =
            '${parsedMembershipPriceById - parsedUserDiscount} ₽';
        return amount;
                        ...
                        return SingleChildScrollView(
                          child: Column(
                            children: <Widget>[
                              ...
                              sized10,
                              _buildMembershipButton(context, 1,
                              ...
                              sized10,
                              _buildMembershipButton(context,
                              ...
                              sized10,
                              _buildMembershipButton(context, 3,
                                  MembershipType.elite, phone, isElite: true),
                              sized10,
                              _buildCatalogButton(
                                context,
                                3,
  ...
  Widget _buildMembershipButton(BuildContext context,
      int id, MembershipType type, String phone, {bool isElite = false}) {
    bool isAvailable = context.read<MembershipCubit>().isAvailable(id);
    return CstmBtn(
      onTap: isAvailable
          ? () async {
        final String? url = await PaymentRepo.getWeblink(id,
            context.read<MembershipCubit>().getPrice(id));
        ...
        if (url != null && context.mounted) {
              ...
              'onComplete': () async {
                ...
                Future.delayed(
                  Duration.zero,
                    ...
                    context.read<ProductCubit>().load(isPublic:
          ...
          : () {},
      text: checkMembershipPrice(context.read<MembershipCubit>().getPrice(id))!,
      textStyle: Theme.of(context).textTheme.titleMedium?.copyWith(
        fontSize: 20,
        color: isElite ? (isAvailable ?
        Theme.of(context).scaffoldBackgroundColor : Colors.grey) :
  ...
  Widget _buildCatalogButton(BuildContext context,
      int id, String text, String extra, Color color, Color borderColor) {
    ...
    return CstmBtn(
      onTap: isAvailable
        ...
        context.pushNamed(
          ...
          pathParameters: <String, String>{'type':
          MembershipType.values[id - 1].name},
  ...
  Color _getButtonColor(BuildContext context, int id, bool isAvailable) {
    if (id == 1) {
      ...
      return isAvailable ? Theme.of(context).primaryColor :
      Colors.grey.withOpacity(0.5);
    } else if (id == 3) {
      return isAvailable ? Theme.of(context).primaryColorDark :
      Colors.grey.withOpacity(0.5);
    } else {
