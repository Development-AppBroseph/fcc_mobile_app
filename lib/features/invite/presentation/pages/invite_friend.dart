...
class _InviteFrPageState extends State<InviteFrPage> {
  ...
  void initState() {
    super.initState();
    _checkInvitePermission();
  }

  Future<void> _checkInvitePermission() async {
    final cubit = context.read<InvitationCubit>();
    final authCubit = context.read<AuthCubit>();
    if (authCubit.state is Authenticated) {
      final result = await cubit.checkCanInvite();
      setState(() {
        canInvite = result;
        isLoading = false;
      });
                  ...
                  child: BlocBuilder<AuthCubit, AuthState>(
                    builder: (BuildContext context, AuthState auth) {
                      return auth is Authenticated
                        ...
                        children: <Widget>[
                          sized20,
                          CustomBackButton(path: RoutesNames.introCatalog),
                          Expanded(
                            child: Column(
                ...
                if (isLoading)
                  ...
                  BackdropFilter(
                    ...
                    child: Container(
                      ...
                      child: const Center(
                        child: Text(
                          'Чтобы поделиться ссылкой для приглашения, необходимо осуществить покупку плана',
